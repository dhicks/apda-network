...
# Keep only programs with validated data
grads <- grads_unfltd %>%
...
keywords_unfltd <- read_excel(path = file.path(data_folder, 
                                        '00_AOSDatax_2021-08-19.xlsx'), 
                       sheet = 'Keyword') %>% 
  slice(-1) %>% 
...
cluster.complete <- agnes(x = distance.matrix, 
                          diss = TRUE, 
                          method = 'ward', 
                          metric = NULL)
...
# k = 2 ----
process_dendro(complete.data, dendrogram, 2)
k2 = process_dendro(complete.data, dendrogram, 2)
...
# k = 3 ----
k3 = process_dendro(complete.data, dendrogram, 3)
...
cluster_table = university.and.cluster %>% 
  ...
  kable(format = 'latex', 
        ...
        caption = 'Programs and clusters.  Programs are listed alphabetically within their $k=5$ cluster.')
